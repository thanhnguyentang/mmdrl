...
from dopamine.utils import atari_lib
import gym
import numpy as np
...
BasicImplicitQuantileNetworkType = collections.namedtuple(
    'basic_iqn_network', ['quantile_values', 'quantiles'])
...
gin.constant('gym_lib.CARTPOLE_MIN_VALS', CARTPOLE_MIN_VALS)
gin.constant('gym_lib.CARTPOLE_MAX_VALS', CARTPOLE_MAX_VALS)
gin.constant('gym_lib.ACROBOT_MIN_VALS', ACROBOT_MIN_VALS)
gin.constant('gym_lib.ACROBOT_MAX_VALS', ACROBOT_MAX_VALS) 
...
@gin.configurable
class BasicImplicitQuantileNetwork(tf.keras.Model):
  """The Implicit Quantile Network (Dabney et al., 2018).."""

  def __init__(self, num_actions, quantile_embedding_dim, min_vals, max_vals, name=None):
    ...
    Args:
      ...
      name: str, used to create scope for network parameters.
    """
    super(BasicImplicitQuantileNetwork, self).__init__(name=name)
    self.num_actions = num_actions
    ...
    # Defining layers.
    self.flatten = tf.keras.layers.Flatten()
    self.dense1 = tf.keras.layers.Dense(512, kernel_initializer=self.kernel_initializer, activation=self.activation_fn, name='fully_connected')
    self.dense2 = tf.keras.layers.Dense(512,  kernel_initializer=self.kernel_initializer, activation=self.activation_fn, name='fully_connected')
  ...
  def call(self, state, num_quantiles):
    ...
    x = tf.cast(state, tf.float32)
    x = self.flatten(x) 
    x = (x -self.min_vals) / (self.max_vals - self.min_vals)
    x = 2.0 * x - 1.0 
    x = self.dense1(x)
    ...
    x = self.dense2(x)
    quantile_values = self.last_layer(x)
    return BasicImplicitQuantileNetworkType(quantile_values, quantiles)