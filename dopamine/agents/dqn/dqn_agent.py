...
class DQNAgent(object):
  ...
  def _build_networks(self):
    ...
    # TODO: Ties should be broken. They are unlikely to happen when
  ...
  def begin_episode(self, observation):
    ...
    self.action = self._select_action() # a_t 
    return self.action
  ...
  def end_episode(self, reward):
    ...
    Args:
      reward: float, the last reward from the environment. r_T 
    """
    if not self.eval_mode:
  ...
  def _train_step(self):
    ...
    if self._replay.memory.add_count > self.min_replay_history:
      if self.training_steps % self.update_period == 0:
        _, loss = self._sess.run(self._train_op)
        # self.statistics_collection.append(loss)
